        ...
        for task, dataset in datasets.items():
            ...
            if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA"]:
                ...
                if not args.test_limit:
                    ...
                    for split, lines in dataset.items():
                        ...
                        if args.test_limit < len(lines):
                            temp_random = random.Random(50)
                            lines = temp_random.sample(dataset[split], args.test_limit)
                ...
                if not args.test_limit or args.test_limit >= len(dataset['test']):
                    ...
                    dataset['test'].sample(args.test_limit, random_state=50).to_csv(os.path.join(setting_dir, 'test.csv'), header=False, index=False)
                ...
                with open(os.path.join(setting_dir, "train.tsv"), "w") as f:
                    ...
                    if args.balance:
                        ...
                        for i in range(extra):
                            class_num += 1
                            lines.append(label_list[list(label_list.keys())[class_num]][base])
...
def prepro_for_zhang(dataname, split, seed, args):
    ...
    with open(save_path, "w") as f:
        ...
        if args.balance and split != "test":
            ...
            for i in range(extra):
                class_num += 1
                lines.append(data[list(data.keys())[class_num]][base])
            ...
            for sents in data.values():
                if split=="test":
                    if args.test_limit > len(sents):
                        temp_random = random.Random(50)
                        temp_random.shuffle(sents)
                elif balance:
                    sents = sents[:k]
                else:
                    sents = sents[:k]
